...
import { Gpu, Plus, Edit2, Trash2, X, Search } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">GPU</h1>
          <p className="text-sm text-slate-400 mt-1">
            Manage graphics processor database
          </p>
        </div>
        <button
      ...
      <div className="rounded-lg border border-purple-500/20 bg-[#0F172A]/50 backdrop-blur-sm overflow-hidden">
        {loading && gpus.length === 0 ? (
          <div className="flex items-center justify-center py-12">
            <p className="text-slate-400">Loading GPUs...</p>
          </div>
        ) : filteredGPUs.length === 0 ? (
          <div className="flex items-center justify-center py-12">
            <p className="text-slate-400">
              {gpus.length === 0 ? "No GPUs added yet." : "No results found."}
            </p>
          </div>
        ) : (
                    ...
                    <td className="px-6 py-4 text-sm text-white">
                      <div className="flex items-center gap-3">
                        <div className="rounded-lg bg-purple-500/20 p-2">
                          <Gpu className="h-4 w-4 text-purple-400" />
                        </div>
